...
# Except for negative values should be -1 but returns 2
print(-10 % 3)
...
# Can be used as key for hash map and Hash set unlike lists
myMap = { (1,2): 3 }
print(myMap[(1,2)])
...
while len(minHeap):
    print(heapq.heappop(minHeap)) # O(log n)
...
# Build heap from initial values - TC O(N) for heapify
arr = [2, 1, 8, 4, 5]
heapq.heapify(arr) 
while arr:
...
def double(arr, val):
    def helper():
        ...
        # Can not be modified unless specified nonlocal
        # val *= 2
