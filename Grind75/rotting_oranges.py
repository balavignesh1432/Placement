...
# Use fresh counter to determine possible or not, as well as time taken when no fresh left
# Each level of BFS indicates one minute passed
...
def orangesRotting(self, grid: List[List[int]]) -> int:
    ...
    # Count fresh oranges and collect initial rotten ones
    ...
    q = deque(rotten)
    minutes = 0
    while q and fresh:  # Do until there are no fresh oranges and q not empty, Q could be non empty but no fresh left
        for _ in range(len(q)):
            ...
            for rd, cd in directions:
                ...
                if r < 0 or c < 0 or r >= len(grid) or c >= len(grid[0]) or grid[r][c] != 1:
                    continue
                grid[r][c] = 2          # Mark as rottern, acts as visited
                fresh -= 1              # Decrease fresh counter
                q.append([r, c])
        minutes += 1

    return minutes if fresh == 0 else -1    # If fresh oranges remain, return -1